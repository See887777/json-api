...
This object **MUST** reserve the `id` and `type` keys at its top level, but
can otherwise contain any valid JSON value.

#### Attributes <a href="#document-structure-resource-object-attributes" id="document-structure-resource-object-attributes"></a>

All members which appear in an "attributes object" are considered attributes.
...
A resource object's [attributes] and [links] keys are collectively called its
